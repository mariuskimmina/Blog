...
![image](/hacks/htb/feline/feline-pwn.png "Feline has been pwned")
...
![image](/hacks/htb/feline/feline-virusbucket.png "Feline Virusbucket")

if we capture the HTTP request for the file-upload, you can use a proxy like burp or ZAP for this, then you will see this:

![image](/hacks/htb/feline/feline-file-upload.png "Feline Upload")
...
![image](/hacks/htb/feline/feline-upload-error.png "Feline Upload Error")
...
```
curl -s --unix-socket /var/run/docker.sock http://localhost/hacks/htb/json
```

If I go ahead and run this one myself we get the following output:

```
root@2d24bf61767c:~# curl -s --unix-socket /var/run/docker.sock http://localhost/hacks/htb/json
<t /var/run/docker.sock http://localhost/hacks/htb/json
